...
class AFTAttention(nn.Module):
    def __init__(self, config):
        super(AFTAttention, self).__init__()
        self.config = config
...
class AFTLayer(nn.Module):
    def __init__(self, config):
        super(AFTLayer, self).__init__()
        self.attention = AFTAttention(config)
...
class AFTEncoder(nn.Module):
    def __init__(self, config, pooler_count=1):
        super(AFTEncoder, self).__init__()
        self.config = config
        self.encoders = nn.ModuleList([AFTLayer(config) for _ in range(config.num_hidden_layers)])
...
class AFT(torch.nn.Module):
    ...
    def __init__(self,config):
        ...
        self.fastformer_model = AFTEncoder(config)
