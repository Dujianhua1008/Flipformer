...
class TransformerEncoder(nn.Module):
    def __init__(self, config, pooler_count=1):
        ...
        self.config = config
        # self.encoders = nn.ModuleList([TransformerLayer(config) for _ in range(config.num_hidden_layers)])
    ...
    def forward(self, 
        ...
        out = self.encoders(embeddings, src_key_padding_mask=extended_attention_mask)
        ...
        output = self.poolers[pooler_index](out, attention_mask)
...
class Transformer(torch.nn.Module):
    ...
    def __init__(self,config):
        ...
        self.fastformer_model = TransformerEncoder(config)
