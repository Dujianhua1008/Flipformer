...
from transformers.models.big_bird.modeling_big_bird import BigBirdEncoder as bigbirdencoder
...
#         self.config = config
#         # self.encoders = nn.ModuleList([BigBirdLayer(config, seed=0) for i in range(config.num_hidden_layers)])
#         self.encoders = bigbirdencoder(
#             config=config,
#         )
...
class BigBird(torch.nn.Module):
    ...
    def __init__(self,config):
        ...
        self.fastformer_model = BigBirdModel(config, add_pooling_layer=False)
    ...
    def forward(self,input_ids,labels):
        ...
        pooler_out = self.pooler(text_vec['last_hidden_state'])
        score = self.dense_linear(pooler_out)
