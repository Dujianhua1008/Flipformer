...
from linformer import Linformer as linformer
...
#     def __init__(self, config, seq_len, k):
...
#         self.attention = LinformerAttention(config, seq_len, k)
...
class LinformerEncoder(nn.Module):
    def __init__(self, config, seq_len, k, pooler_count=1):
        super(LinformerEncoder, self).__init__()
        self.config = config
        # self.encoders = nn.ModuleList([LinformerLayer(config, seq_len, k) for _ in range(config.num_hidden_layers)])
        self.encoders = linformer(
            dim=config.hidden_size,
            seq_len=seq_len,
...
class Linformer(torch.nn.Module):
    ...
    def __init__(self,config, seq_len, k):
        ...
        self.fastformer_model = LinformerEncoder(config, seq_len, k)
